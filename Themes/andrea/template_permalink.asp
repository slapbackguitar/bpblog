...
<div id="content" class="group">
  <h2 id="post-<%=(rsArticles.Fields.Item("BlogID").Value)%>"><a href="template_permalink.asp?id=<%=(rsArticles.Fields.Item("BlogID").Value)%>" title="Permalink for <%=(rsArticles.Fields.Item("BlogHeadline").Value)%>"><%=(rsArticles.Fields.Item("BlogHeadline").Value)%><% if Session("MM_Username") <> "" AND Session("MM_UserID") = rsArticles.Fields.Item("fldAuthorID").Value OR Session("MM_isAdmin") = 1 Then %> | <a href="update_blog.asp?passID=<%=(rsArticles.Fields.Item("BlogID").Value)%>">Edit this post</a><% end if %></a><a name="<%=(rsArticles.Fields.Item("BlogID").Value)%>" id="<%=(rsArticles.Fields.Item("BlogID").Value)%>"></a></h2>
...
<p id="<%=(rsComments.Fields.Item("commentID").Value)%>"><strong><%=CommentCount%>.</strong> <%=MakeHyperlink(rsComments.Fields.Item("commentHTML").Value)%></p>
</div>
<div class="commentAuthor">
<% If  Len(rsComments.Fields.Item("commentURL").Value) > 12 Then %>
<p><strong><a href='<%=(rsComments.Fields.Item("commentURL").Value)%>' rel='external nofollow'><%=(rsComments.Fields.Item("commentName").Value)%></a></strong></p>
<% Else %>
<p><strong><%=(rsComments.Fields.Item("commentName").Value)%></strong>
<% End If %>
<p><small><a href="#<%=(rsComments.Fields.Item("commentID").Value)%>" title="Comment Permalink"><%=(rsComments.Fields.Item("commentDate").Value)%></a></small></p>
	</div>
	<div class="clear"></div>
</li>
<% 
