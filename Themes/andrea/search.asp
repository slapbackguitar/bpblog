...
%>
<h2 id="post-<%=(rsSearch.Fields.Item("BlogID").Value)%>"><a href="template_permalink.asp?id=<%=(rsSearch.Fields.Item("BlogID").Value)%>" title="Permalink for <%=(rsSearch.Fields.Item("BlogHeadline").Value)%>"><%=highlight((rsSearch.Fields.Item("BlogHeadline").Value),searchterm)%></a></h2>
  <div class="stamp">Posted at <%= DoDateTime((rsSearch.Fields.Item("BlogDate").Value), 3, 1033) %> in <a href="template_archives_cat.asp?cat=<%=(rsSearch.Fields.Item("CatID").Value)%>" title="<%=(rsSearch.Fields.Item("CatDesc").Value)%>"><%=(rsSearch.Fields.Item("CatName").Value)%> (<%=(rsSearch.Fields.Item("CategoryCount").Value)%>)</a></div>
  <div class="main">
  <% if (rsSearch.Fields.Item("BlogReadMore").Value) = 1 Then %>
  <p><%=CropSentence(CI_StripHTML(rsSearch.Fields.Item("BlogHTML").Value), 500, "...")%></p>
  <h4 align="center"><a href="template_permalink.asp?id=<%=(rsSearch.Fields.Item("BlogID").Value)%>#<%=(rsSearch.Fields.Item("BlogID").Value)%>" title="Read More <%=(rsSearch.Fields.Item("BlogHeadline").Value)%>">Read More "<%=(rsSearch.Fields.Item("BlogHeadline").Value)%>"</a></h4>
  <% Else %>
  <%=readmore(rsSearch.Fields.Item("BlogHTML").Value,rsSearch.Fields.Item("BlogID").Value)%>
  <% End If %>
  </div>
  <div class="meta">
  <p>Written by <a href="template_author.asp?id=<%=(rsSearch.Fields.Item("fldAuthorID").Value)%>" title="<%=(rsSearch.Fields.Item("fldAuthorRealName").Value)%>'s Profile"><%=(rsSearch.Fields.Item("fldAuthorRealName").Value)%></a> on <%= DoDateTime((rsSearch.Fields.Item("BlogDate").Value), 1, 1033) %></p>
  </div>
  <% 
